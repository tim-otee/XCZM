...
def contact(msg):
	...
	fromaddr = 'crump.timothy@gmail.com'
	toaddrs  = 'timothy.crump@manchester.ac.uk'

	# Credentials (if needed)
	username = 'codeastermodelfinished'
	password = 'codeastermodelfinished1'
...
def post_multi_files():
	...
	os.chdir(REPE)
	for file in glob.glob("*.rmed"):
		print(file)
		rmeds_list.append(file)
	print rmeds_list
	
	count1=len(rmeds_list)
	...
	SALOME=os.path.join(os.path.expanduser('~'), '/home/mbgnktc2/salome_meca/appli_V2016') #Salome installation path
	...
	for i in range(0,int(count1)): 
		...
		IN="INPUT="+"'"+REPE+"/"+rmeds_list[i]+"'"
		OUT="OUTPUT='"+WORKING+"/"+rmeds_list[i]+".png'"
...
def concatinate_csv():
	...
	################################
	for z in range(0,N):
		file=name[z]+'.'+str(0)+'.csv' #file='results0.0.csv'
...
def mem():
    ...
    with open(new_file, "r+") as f:
     while (TEST==True): 
         ...
         #f.write(old+str(t)+' \n') # write the new line before
         words=t.split()
         Memory.append(words[4])
         time.sleep(refresh)
         i=i+1
         os.system('cls' if os.name == 'nt' else 'clear')
         print("------------------------------------------------------------------------------")
         print "Running memory saving with interval "+str(refresh)+"\n"
         print "saving to data (CSV) and image to : \n "+ str(WORKING)+" \n "
         print "Inst =  "+str(i)+'\r'
         print("------------------------------------------------------------------------------")
